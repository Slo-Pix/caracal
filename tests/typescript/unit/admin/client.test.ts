...
describe('AdminClient', () => {
  ...
  it('covers the API namespace surface with correct path and method', async () => {
    ...
    await c.teams.list('z1')
    await c.teams.get('z1', 'team1')
    await c.teams.create('z1', { name: 'Ops' })
    await c.teams.patch('z1', 'team1', { members: [{ id: 'user-1', role: 'owner' }] })
    await c.teams.delete('z1', 'team1')
    await c.invitations.list('z1')
    await c.invitations.create('z1', { email: 'dinesh@piedpiper.example', role: 'admin', invited_by: 'user-1' })
    await c.invitations.cancel('z1', 'invite1')
    ...
    expect(calls.some((x) => x.url === 'http://api/v1/zones/z1/teams/team1' && x.method === 'PATCH')).toBe(true)
    expect(calls.some((x) => x.url === 'http://api/v1/zones/z1/invitations/invite1' && x.method === 'DELETE')).toBe(true)
