...
//
// Unit tests for the coordinator relay entry point.
...
func TestRunReturnsZeroAndShutsDownTelemetry(t *testing.T) {
	...
	setupTelemetry = func(_ context.Context, service string) (func(context.Context) error, error) {
		if service != "caracal-coordinator-relay" {
...
func TestRunTreatsContextCancellationAsSuccess(t *testing.T) {
	resetHooks(t)
	newRunner = func(context.Context) (runner, error) {
		return fakeRunner{err: context.Canceled}, nil
	}
...
func TestRunReturnsOneWhenRunnerFails(t *testing.T) {
	resetHooks(t)
	newRunner = func(context.Context) (runner, error) {
		return fakeRunner{err: errors.New("stream failed")}, nil
	}
