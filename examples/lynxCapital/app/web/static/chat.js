...
function renderMessage(type, data = {}) {
  try {
    ...
    fallbackNode.querySelector('.msg-system-text').textContent =
      `Error rendering telemetry: ${error.message}. Payload: ${JSON.stringify(data)}`
...
function loadMockSession(sessionId) {
  ...
  document.dispatchEvent(
    new CustomEvent('mock:graph-load', {
      detail: data.graph,
    }),
  )
