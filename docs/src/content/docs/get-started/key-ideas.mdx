...
Gateway resources can also bind to a provider when the upstream API requires provider-native authentication. Internal Caracal-aware resources use the same Gateway path with the Caracal mandate forwarded to the upstream verifier; provider-native credentials stay inside the trusted Gateway/STS boundary.
