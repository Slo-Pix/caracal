...
2. Save it at the profile path shown by the web console.
3. Copy the one-time app client secret into the secret-file path shown by the web console.
4. Restrict the secret file so only the current user can read it.
5. Rerun the first-success command from the result view.

The app client secret is shown once for newly created apps. If you lose it, create a new secret from the web console and update the profile.
...
| Wrong path | Use the first request path from the web console result view before trying custom paths. |
