...
| `CARACAL_SUBJECT_TOKEN` | Obtained at runtime | Ambient mandate for this agent when using static-token startup |

The subject token is not a static secret — it is an ambient mandate obtained by exchanging the application's client credentials with the STS. The SDK's `fromEnv()` factory handles this transparently when used with `caracal run`. For long-running services, TypeScript, Python, and Go can also read `CARACAL_APP_CLIENT_SECRET` plus `CARACAL_STS_URL` and refresh the application subject token on demand.
...
The middleware extracts the agent's mandate from the `Authorization` header and W3C baggage, then binds the `CaracalContext` to the request. Use the MCP Express connector only when you are protecting an MCP server and need JWT verification plus scope checks at the transport boundary.
