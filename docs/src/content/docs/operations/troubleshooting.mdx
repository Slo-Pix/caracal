...
The web console **Request Traces** is the fastest disambiguator: it ties one request ID to the resource, subject, scopes, determining policies, diagnostics, and result, so you can see whether the denial came from STS policy or from a resource verifier.
...
3. **Trace the request.** Open web console **Request Traces** with the request ID. It shows the resource, subject, requested scopes, determining policies, and result.
