...
5. Enforce per-call token use by default, plus `requiredScopes`, `requiredTargets`, `requireAgent`, `requireDelegation`, `requireChainContains`, `maxHopCount`, and active-execution expiry.
...
interface AuthDeps {
  ...
  requiredUse?: string;                 // Default: 'per_call'
