---
title: Terminal Management Interface
description: Navigate caracal-terminal — launch, screen layout, key bindings, views, and live audit streaming.
pageType: reference
---

`caracal-terminal` is the terminal management interface for inspecting and managing Caracal entities without leaving the shell. It fetches from the control-plane API and renders into a full-screen terminal buffer with keyboard navigation.
...
On startup, the terminal management interface enters full-screen alt buffer mode and hides the cursor. Press `q` or `ctrl-c` at any time to exit and restore the terminal.
