...
When `auth_mode` is `provider_oauth` or `provider_apikey`, the inbound `Authorization` header is removed before forwarding and the Caracal mandate is exposed only as `X-Caracal-Identity: <mandate>` for Caracal-aware upstream sidecars.
