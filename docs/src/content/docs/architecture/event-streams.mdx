...
At startup the Redis container entrypoint runs `provision-streams.sh` after Redis accepts commands. The Helm deployment uses the same script as the Redis bootstrap path before application services consume streams. For each stream and consumer group:
