...
- **The web BFF drives every operator session with the shared global admin token.**
  - *The limit.* `apps/auth` resolves one global admin (and one coordinator) token server-side and uses it for every signed-in operator's proxied calls; authentication is not yet tiered into per-operator *authority*. So while only allowlisted operators can sign in, each one wields the same global admin token rather than a zone-scoped per-operator one.
  - *How Caracal contains it.* Registration is gated as an authority boundary: `databaseHooks.user.create.before` rejects any identity outside `CARACAL_OPERATOR_EMAILS` (exact emails or `@domain` suffixes) on both email/password and social sign-up, and with no allowlist registration is open only in development and fails closed in production. The auth surface is session-gated, origin-enforced, header-hardened, and rate-limited on credential endpoints before any proxy call (T13); the global token is never exposed to the browser. Every state-changing proxied action is logged server-side from the validated session as an `operator action` carrying the operator id/email and the request id; that request id is forwarded downstream and recorded as the admin-audit `request_id`, so each tamper-evident audit row joins back to the authenticated operator even though the proxy uses the shared token. Because the operator is bound from the session rather than a request field, an operator cannot attribute an action to anyone else.
  - *Path to closure.* Mint a zone-scoped per-operator admin token through `POST /v1/admin-tokens` for each operator instead of reusing the shared global token, so the control plane enforces per-operator authority directly rather than relying on the BFF join for attribution.
...
- **How Caracal handles it.**
  ...
  - *Authority is authenticated, not yet authorized per operator.* The BFF proxies every signed-in session with the shared global admin token and Community Edition sign-up is open by default, so a registered operator holds full cross-zone authority and the admin-audit ledger attributes their actions to the shared token. Restricting registration, issuing zone-scoped per-operator tokens, and forwarding operator identity for attribution are tracked in [Known Limits and How Caracal Contains Them](#known-limits-and-how-caracal-contains-them).
